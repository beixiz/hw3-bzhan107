...
padZero l1 l2 = if (length l1 < length l2) then (((clone 0 (abs((length l1) - (length l2)))) ++ l1), l2)
  else
  if (length l1 > length l2) then (l1, ((clone 0 (abs((length l1) - (length l2)))) ++ l2))
  else
  (l1, l2)
...
removeZero [] = []
removeZero (x:xs) = if(x == 0) then (removeZero xs)
                    else x : xs
...
bigAdd l1 l2     = removeZero res
  where
    ...
    f a x        = ((fst (a) + fst (a) + snd (x)) `div` 10, ((fst (a) + fst (x) + snd (x)) `mod` 10): snd (a))
    base         = (0, [])
...
mulByInt i n = removeZero res
  where
    res = snd (foldLeft f base args)
    f a x = ((fst (a) + (i*x)) `div` 10, ((fst (a) + (i*x)) `mod` 10) : snd (a))
    base = (0,[])
    args = reverse([0] ++ n)
